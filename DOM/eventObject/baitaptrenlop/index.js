var lyric =
    // 20240808215423
    // https://zingmp3.vn/api/v2/lyric/get/lyric?id=ZWB0DOFI&BGId=0&ctime=1723128837&version=1.10.46&sig=66d89c2b8074bb6657af49c37fcff4d4a5f35fa628715ec5574315fdc90dec492ef4542effb9d4066d73b6a948e9ef73cbab1f8f61ed11c1b6d9484113930f98&apiKey=X5BM3w8N7MKozC0B85o4KMlzLZKhV00y
...
var karaInner = document.querySelector(".karaoke-inner");
var rate = 0;
...
var pendingSong = document.createElement("div");
pendingSong.innerHTML = "<p>Em Ơi Anh Khóc Thật Rồi<p><p>Ca sĩ: Quốc Thiên</p>";

var sentencesArr = lyric.data.sentences;
var newArr = [];
sentencesArr.forEach(function (item) {
    newArr.push(item.words);
});

var getWord = function (currentTime) {
    var index = newArr.findIndex(function (item) {
        if (
            currentTime >= item[0].startTime &&
            currentTime <= item[item.length - 1].endTime
        ) {
            return true;
        }
    });

    var firstWordsStartTime = newArr[0][0].startTime;
    var lastSentence = newArr[newArr.length - 1];
    var lastWordEndTime = lastSentence[lastSentence.length - 1].endTime;
    var bufferTime = 5000;

    if (
        currentTime < firstWordsStartTime - bufferTime ||
        currentTime > lastWordEndTime + bufferTime
    ) {
        karaInner.innerText = "";
        karaInner.append(pendingSong);
    }

    if (index !== -1) {
        var currSentence = newArr[index];
        var nextSentence = newArr[index + 1];
        var longPauseDuration = 10000;

        if (
            nextSentence &&
            currSentence[currSentence.length - 1].endTime -
            ...
            if (index >= offset && index < offset + 2) {
                for (var i = offset; i < offset + 2; i++) {
                    ...
                    newArr[i].forEach(function (item, wordIndex) {
                        var spanWord = document.createElement("span");
                        spanWord.classList.add("word");
                        spanWord.dataset.index = wordIndex;
...
// Xử lý karaoke
var btnKaraoke = document.querySelector(".open-karaoke");
...
var arrowDown = document.querySelector(".karaoke-header .close");
